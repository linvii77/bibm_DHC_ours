...
完整四指标表（Dice / HD95 / NSD / ASD）见 **`results/REPRODUCTION_RESULTS.md`**。

| 数据集 | 测试集 | Mean Dice | Mean HD95 | Mean NSD | Mean ASD |
|--------|--------|-----------|-----------|----------|----------|
| Synapse 20% (3-fold) | 6 cases | **47.03 ± 1.31%** | 18.82 ± 2.32 mm | 54.90 ± 1.83% | 8.46 ± 2.45 mm |
| AMOS 5% (fold1) | 120 cases | **40.11%** (官方) / 42.75% (HD95脚本) | 55.34 mm | 36.52% | 37.99 mm |
...
重新计算 HD95/NSD：`python scripts/evaluate_hd95_nsd.py`（需已有 `logs/.../predictions_AB/`）
...
- `weights/` — Synapse 3-fold + AMOS 5% fold1 best checkpoint（Git LFS，共 4 个 × 289MB）
- `splits/` — 数据划分
- `results/REPRODUCTION_RESULTS.md` — **完整复现结果表**（Dice / HD95 / NSD / ASD）
- `results/metrics_hd95_nsd_summary.txt` — HD95/NSD 机器可读汇总
- `results/evaluation_res.txt` — Synapse 官方评估（Dice 47.03%）
- `results/amos_5p_fold1_*` — AMOS fold1 评估与摘要
- `scripts/evaluate_hd95_nsd.py` — 从预测 nii 计算 HD95/NSD

## 8. AMOS 5% labeled（fold1 本地复现）

### 数据

1. 下载 AMOS22 预处理数据（官方）：  
...
```bash
git lfs pull
bash scripts/setup_amos_weights.sh
...
| 指标 | 值 |
|------|-----|
| 验证集 best Dice (eval, 24) | 37.44% @ epoch 270 |
| **测试集 Mean Dice (test, 120)** | **40.11%** |

详见 `results/amos_5p_fold1_SUMMARY.md`、`results/amos_5p_fold1_evaluation_res.txt`。

### 从头训练 fold1（可选）

```bash
bash train3times_seeds_20p.sh -c 0 -t amos -m dhc -e '' -l 3e-2 -w 0.1 -p 5
# 仅跑 fold1 时可在第一个 fold 完成后手动停止
```

训练进度：`python3 scripts/watch_train_progress.py`
